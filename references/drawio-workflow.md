...
- **fidelity level (L1/L2/L3) and iteration budget — see `SKILL.md` Step 0; this is a blocking gate: ask the user and wait for their answer (or an explicit delegation) before authoring anything**
- **delivery strategy — draft-first (default) or polish-first; under draft-first, reveal the file + preview right after the first pre-flight pass instead of waiting for the review cycle**
...
## 4.5 Quality Checks: Quick (Draft) And Full (Finalization)

**Two depths, matched to the phase:**
- **Quick check (draft phase)** — `validate_visual_quality.py --quick`: display-blocking rules only (overlap, arrow-through-box, severe text overflow). Run it before the first preview and after every draft-phase change. Small findings wait for finalization.
- **Full check (finalization)** — `validate_visual_quality.py`: all rules. Required at L2/L3, recommended at L1. Fix FAILs or waive with a logged reason, review every WARN, and compile the findings into the prioritized issue list for the user.
...
**In the full check, fix every FAIL** (or waive a specific rule with `--waive <rule>` and log the reason — common at L1, unusual at L2, rare at L3), **and review every WARN.** If the checker exits non-zero, fix the issues and re-run before regenerating the preview. Load `references/xml-preflight.md` for the full explanation.
...
For an agent-side canvas-only screenshot (no visible browser needed):

```powershell
python .\scripts\render_png.py D:\path\figure.drawio
```

It renders via headless system Edge/Chrome, auto-fits the viewport to the diagram, and writes `<file>.png`. Run `python .\scripts\preflight.py` once per session to confirm the channel is available.
...
**Loop rules (scaled by fidelity level — the Gate Matrix in `SKILL.md` is authoritative):**

1. **Style extraction before authoring** — if reference images are provided as style guides (compact at L2, full at L3).
2. **Pre-flight before first preview** — fix FAILs, or waive a specific rule with a logged reason.
3. **Bounded cycles.** At most the level default (L1: 1, L2: 2, L3: 3) or the user's budget. Stop as soon as the exit checklist passes — even after one cycle.
4. **Honest zone review.** Scan the zones in scope (L1: zones 1–4 and 7; L2/L3: all nine). Record real findings only; a zone with no findings is fine. Never fabricate entries.
5. **Fix all P0/P1 in scope per cycle** (L1 fixes P0 only).
6. **Fix verification after each fix pass.** Compare old vs new screenshot at each defect location. Mark FIXED/NOT FIXED/PARTIAL/REGRESSION. Two failed attempts on the same P0/P1 → report to the user.
7. **Red-team pass (L3, once, budget permitting).** No finding quotas. Skip it honestly if the budget is spent and say so.
...
1. extract style from references (if provided) — to the depth the level requires
2. run pre-flight (`validate_visual_quality.py`) — fix FAILs or waive with a reason
3. write or patch `.drawio`
4. regenerate preview HTML
5. refresh browser and capture a canvas-only screenshot (diagram content ≥80% of image)
6. scan the zones in scope; record findings honestly
7. fix all P0/P1 in scope
8. regenerate preview HTML and take a new screenshot; verify each fix (FIXED/NOT FIXED/PARTIAL/REGRESSION)
9. append `defect-log.md` (L2 with a log / L3)
10. check the exit checklist: pass → hand off; not pass and budget remains → next cycle (ask the user first in Interactive mode); budget spent → hand off with the gap list
11. run validators
...
Each cycle is: **screenshot → scan → fix P0/P1 in scope → verify → log**.

1. Refresh the local preview URL (with cache bust).
2. Screenshot (canvas-only).
3. Scan the zones in scope and record real findings before fixing anything. If unsure whether the scan was complete, cross-check against five audit dimensions: requirements, semantics, visual hygiene, style, regression.
4. Fix all P0/P1 in scope.
5. Regenerate the preview HTML and take a new screenshot.
6. Verify each fix by comparing old vs new at each location: FIXED / NOT FIXED / PARTIAL / REGRESSION. A NOT FIXED P0/P1 gets one retry; a second failure → report to the user.
7. Append the pass and verification to `defect-log.md` (L2 with a log / L3); do not overwrite earlier review rows.
8. Check the exit checklist for the level: pass → hand off; budget remains → next cycle (ask the user first in Interactive mode); budget spent → hand off with the gap list.
...
Blocker categories that prevent handoff (any level):
...
- (L2/L3) pre-flight not passed and no waivers logged
- any P0/P1 in scope that failed verification twice without being reported to the user

Blockers are handled by fixing them, or — when the budget is spent or the user accepts the tradeoff — by listing them in the gap list with reasons.
...
- side-by-side reference vs result (style/replication tasks)
- local preview URL if the server is still running
- validation summary (`validate_drawio.py` + `validate_visual_quality.py`)
- defect summary (P0/P1 fixed, remaining P2s)
- gap list: remaining visual gaps, unverified claims, unresolved blockers with reasons
- self-score card only if you filled it in (optional)

If exact icons/logos were approximated, state that clearly and identify which assets would close the gap in a future pass.

Do not describe a diagram as complete while the latest screenshot contains a visible P0/P1 that you have not either fixed or listed in the gap list.
