...
This protocol applies whenever the user provides reference images as style sources (not for exact replication — that's `reference-replication-protocol.md`). Depth scales with the fidelity level: a one-line style statement at L1, the compact subset (palette, typography, spacing rhythm) at L2, and the full table below at L3.

## The Rule

**Extract before you draw.** At L3, do not draw a single `<mxCell>` until the full table below is filled. At L2, fill the compact subset. If you cannot fill a row, you do not understand the reference well enough to draw — look again, or ask the user.
