...
Required at L2/L3, recommended at L1. **After writing `.drawio` XML and before generating the first preview HTML, run:**

```powershell
python <skill-dir>/scripts/validate_visual_quality.py <file>.drawio --json --output preflight-report.json
```

Read the JSON report. Do not proceed to preview until:
- Every FAIL is fixed — or waived with `--waive <rule>` and a logged reason (routine at L1, unusual at L2, exceptional at L3)
- Every WARN has been reviewed and either fixed or documented with a reason why it is acceptable
...
The pre-flight catches **computable** defects. The screenshot loop catches **perceptual** defects. Use both — the depth of each scales with the fidelity level.
