...
Usage:
  ...
  python validate_visual_quality.py diagram.drawio --quick    (draft phase: display-blocking rules only)
  python validate_visual_quality.py diagram.drawio --waive arrow-box-collision
      (downgrade specific FAIL rules to non-blocking WAIVED findings; log the reason)
"""
...
# Windows consoles default to a narrow codepage; force UTF-8 so symbols like
# "×" and em dashes print correctly instead of turning into mojibake.
...
@dataclass
class Finding:
    severity: str  # FAIL | WARN | WAIVED (WAIVED = a FAIL downgraded via --waive)
    rule: str
    element_id: str
    message: str
...
def parse_drawio(path: Path) -> Tuple[List[Vertex], List[Edge], float, float]:
    ...
    # --- Resolve nested coordinates to absolute (swimlane children, groups) ---
    # Children of a container store coordinates relative to their parent;
    # all geometry checks below work in absolute canvas space.
    raw_geom = {v.id: (v.rect.x, v.rect.y, v.parent) for v in vertices}
    for v in vertices:
        ax, ay = v.rect.x, v.rect.y
        parent = v.parent
        guard = 0
        while parent not in ("", "0", "1") and parent in raw_geom and guard < 50:
            px, py, next_parent = raw_geom[parent]
            ax += px
            ay += py
            parent = next_parent
            guard += 1
    ...
    # exitX/exitY and entryX/entryY anchor the path on the box edge exactly like
    # draw.io does; a center-to-center approximation would report false
    # arrow-box collisions on any hand-routed (zigzag) diagram.
    vertex_rects = {v.id: v.rect for v in vertices}

    def _anchor(rect: Rect, fx: float, fy: float) -> Tuple[float, float]:
        return (rect.x + fx * rect.w, rect.y + fy * rect.h)

    def _fraction(props: Dict[str, str], key: str) -> float:
        raw = props.get(key)
        try:
    ...
    for edge in edges:
        ...
        if edge.source_point is None and edge.source_id in vertex_rects:
            ...
            if "exitX" in props or "exitY" in props:
                edge.derived_source = _anchor(r, _fraction(props, "exitX"), _fraction(props, "exitY"))
            else:
        ...
        if edge.target_point is None and edge.target_id in vertex_rects:
            ...
            if "entryX" in props or "entryY" in props:
                edge.derived_target = _anchor(r, _fraction(props, "entryX"), _fraction(props, "entryY"))
            else:
...
def _is_lane_or_group(v: Vertex) -> bool:
    """Swimlane frames and group boxes are crossed by edges by design."""
    head = v.style.split(";", 1)[0].strip().lower()
    return head in ("swimlane", "group")
...
def check_arrow_box_collision(
    ...
    findings = []
    # Include all vertices EXCEPT containers and lane/group frames
    # (text, shape, and image are all checked)
    check_targets = [v for v in vertices if not v.is_container and not _is_lane_or_group(v)]
...
def check_font_proportionality(vertices: List[Vertex]) -> List[Finding]:
    findings = []
    for v in vertices:
        if v.rect.h <= 0 or v.is_container or v.is_image or _is_lane_or_group(v):
            continue
...
def check_spacing_variance(vertices: List[Vertex]) -> List[Finding]:
    ...
    for key, indices in y_clusters.items():
        ...
        # Lane children follow the shared column grid: gaps skip empty columns
        # by design, so per-row uniformity does not apply to nested rows.
        nested_row = any(v.parent not in ("", "0", "1") for v in row_items)

        if cv > 0.5 and not nested_row:
            findings.append(Finding(
                severity="WARN",
    ...
    for key, indices in x_clusters.items():
        ...
        # Lane children follow the shared row grid: gaps skip empty rows
        # by design, so per-column uniformity does not apply to nested columns.
        nested_col = any(v.parent not in ("", "0", "1") for v in col_items)

        if cv > 0.5 and not nested_col:
            findings.append(Finding(
                severity="WARN",
...
# Draft-phase quick check: display-blocking defects only. Everything else
# (spacing, palette, typography, decorations, density) waits for finalization.
QUICK_RULES = ("arrow-box-collision", "text-overflow", "element-overlap")
...
def main() -> int:
    ...
    ap.add_argument("--quick", action="store_true",
                    help="Draft-phase quick check: only display-blocking rules "
                         "(" + ", ".join(QUICK_RULES) + ")")
    ap.add_argument("--waive", nargs="*", default=[], metavar="RULE",
                    help="Downgrade FAIL findings of these rules (name or prefix) "
                         "to non-blocking WAIVED entries; record the reason in the defect log")
    args = ap.parse_args()
    ...
    if args.rules:
        ...
        selected = [(rn, rf) for rn, rf in ALL_RULES if rn in QUICK_RULES]
    else:
    ...
    if args.waive:
        for f in findings:
            if f.severity == "FAIL" and any(f.rule.startswith(p) for p in args.waive):
                f.severity = "WAIVED"
    ...
    if args.json:
        report = {
            ...
            "waived": [{"rule": f.rule, "element_id": f.element_id,
                        "message": f.message, "detail": f.detail} for f in waived],
        ...
        print(f"  FAIL: {len(fails)}  |  WARN: {len(warns)}  |  WAIVED: {len(waived)}  |  {pass_str}")
        print()
        ...
        if waived:
            print()
            print("-- WAIVED (downgraded by --waive; record the reason in the defect log) --")
            for f in waived:
                print(f"  [{f.rule}] {f.element_id}: {f.message}")
