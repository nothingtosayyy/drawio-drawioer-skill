...
Note: this installation is a locally customized build (see CHANGELOG.md).
Reinstalling or updating from the upstream repository overwrites local
customizations; merge changes manually instead.
"""
...
def main() -> int:
    ...
    if local_tuple < latest_tuple:
        ...
        print("note: this copy may contain local customizations; updating overwrites them (see CHANGELOG.md)")
        return 1
    if local_tuple > latest_tuple:
        print("status: LOCAL_AHEAD")
        print("note: this is a locally customized build (see CHANGELOG.md)")
        return 0
